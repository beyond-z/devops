...
now=$(date +"%Y%m%d")
db_dump_prod_file=~/dumps/lms_prod_db_dump_$now.sql
pg_dump --clean -h $PORTAL_PROD_DB_SERVER -p 5432 -U $PORTAL_PROD_DB_USER -w -d $PORTAL_PROD_DB_NAME > $db_dump_prod_file
