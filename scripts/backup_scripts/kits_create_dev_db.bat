...
if [ -z "$1" ]
then
  echo "Please pass in the staging DB file to use. E.g.: ./kits_create_dev_db.bat ~/dumps/kits_staging_db_dump_20190909.sql"
  exit 1
fi
...
db_dump_dev_file=~/dumps/kits_dev_db_dump_$now.sql
...
echo "Migrating Kits staging database to dev database"
...
if aws --version 2> /dev/null; then
  aws s3 cp $db_dump_dev_file_gz 's3://kits-dev-db-dumps/kits_dev_db_dump_latest.sql.gz'
  aws s3 cp $db_dump_dev_file_gz "s3://kits-dev-db-dumps/snapshots/kits_dev_db_dump_$now.sql.gz"
  if [ $? -ne 0 ]
  then
